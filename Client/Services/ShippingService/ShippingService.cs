...
namespace LouiseTieDyeStore.Client.Services.ShippingService
{
    public class ShippingService : IShippingService
    {
        ...
        public async Task<ServiceResponse<ShippingResponse>> GetShippingCost(ShippingInfoDTO shippingInfo)
        {
            var result = await _publicClient.PostAsJsonAsync("api/shipping/validate-address", shippingInfo);
            var response = (await result.Content.ReadFromJsonAsync<ServiceResponse<ShippingResponse>>());
